...
def _verified_identity(request: Request) -> tuple[str, Optional[str]]:
    """Cache-key identity must come from the signature-verified Bearer token,
    never from client-supplied X-Tenant-Id/X-User-Id headers - trusting those
    let any caller read or poison another tenant's/user's cached response."""
    auth = request.headers.get("Authorization") or request.headers.get("authorization")
    if auth and auth.lower().startswith("bearer "):
        token = auth.split(" ", 1)[1].strip()
        try:
            claims = jwt.decode(token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
            tenant_id = str(claims.get("tenant_id") or "global")
            user_id = claims.get("sub") or claims.get("user_id")
            return tenant_id, (str(user_id) if user_id else None)
...
def cached(ttl: int = 300, prefix: str = "cache", canonical_path: Optional[str] = None):
    """
    FastAPI Redis Caching Decorator for Users Service.
    Standardized to share the truth with the monolith.
    Supports both sync and async functions.
    Supports canonical_path override.
    """
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Shared key format
            tenant_id, user_id = _verified_identity(request)
            
            # Use canonical path if provided to match monolith gateway
            url_path = canonical_path if canonical_path is not None else request.url.path
            ...
            cache_key = get_cache_key(prefix, tenant_id, user_id, url_path, query_params, body_hash)
